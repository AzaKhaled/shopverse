...
import 'package:shopverse/core/models/base_model.dart';
import 'package:shopverse/core/models/product_model.dart';
...
import 'package:shopverse/features/cart/data/cart_model.dart';
import 'package:shopverse/features/home/presentation/data/banners_model.dart';
import 'package:shopverse/features/home/presentation/data/brands_model.dart';
import 'package:shopverse/features/home/presentation/data/categories_model.dart';
import 'package:shopverse/features/home/presentation/data/category_model.dart';
import 'package:shopverse/features/home/presentation/data/products_model.dart';
import 'package:shopverse/features/login/presentation/data/login_model.dart';
import 'package:shopverse/features/product_details/data/product_details_model.dart';
import 'package:shopverse/features/sign_up/data/register_model.dart';
...
void registerUser() async {
  ...
  final result = await DioHelper.post(
    path: '$baseUrl$apiVersion$registerEndpoint', // auth/register
    token: '', // مفيش توكن هنا
    data: {
  ...
  result.fold(
    ...
    (response) {
      registerModel = RegisterModel.fromMap(response);

      token = registerModel!.token; // خدي التوكن

      saveTokenToSharedPreferences(token: token);

      debugPrint("Register Success => ${registerModel!.token}");
    ...
    result.fold(
      ...
      (r) {
        ...
        // bool a = cartModel!.data.items.any((item) => item.productId == productDetails.id);
         bool a = cartModel?.data.items
        ?.any((item) => item.productId == productDetails?.id) ?? false;
  ...
  void loadTokenFromSharedPreferences() async {
    _token = sl<SharedPreferences>().getString('token') ?? '';
